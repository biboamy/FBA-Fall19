    ...
    def fit(self, tr_loader, va_loader, device):
        ...
        for e in range(1, self.epoch+1):
            ...
            for batch_idx, _input in enumerate(tr_loader):
                ...
                pred = self.model(matrix.view(-1,1,matrix.shape[2], matrix.shape[3]))

                #calculate loss
                loss = loss_func(pred, target.view(-1,1))
                loss.backward()
                opt.step()
            ...
            for batch_idx, _input in enumerate(va_loader):
                ...
                pred = self.model(matrix.view(-1,1,matrix.shape[2], matrix.shape[3]))
                #calculate loss
                loss_val += loss_func(pred, target.view(-1,1)).item()
                r2_val += metrics.r2_score(target.view(-1,1).data.cpu().numpy(), pred.data.cpu().numpy())
